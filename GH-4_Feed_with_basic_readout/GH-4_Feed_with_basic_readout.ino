...
//F1\/
  ...
  //  digitalWrite(KOOLBLOOMDRY,HIGH);  
   ...
   digitalWrite (VEG4,LOW);   //CLOSE THE SOLENOID TO THE RUN
   delay(300000);                   //WAIT TO FLUSH THE BATCH TO THE RUN..CURRENTLY 4 MINUTES 30 SECONDS  
   digitalWrite(VEG3,HIGH);
   digitalWrite(VEG4,HIGH);//CLOSE THE SOLENOID AFTER THE BATCH IS DISTRUBUTED  
   digitalWrite (FLUSH_PUMP,HIGH);
   delay(5000);
   //^VEG3&VEG4 Each getting 1/2 batch
   //VEG2\/ (not active)
 /*digitalWrite(FILL_SOLENOID,LOW);   //open the fill solenoid to fill the reservior.        
   lcd.clear();
   lcd.setCursor(0,0);
   lcd.print ("VEG2");
   lcd.setCursor(0,1);
   lcd.print ("Fill"); delay(60000);      //this is the time it takes to fill the 5 gal bucket enough to prime the mixing pump        
   ...
   lcd.setCursor(0,1);
   lcd.print ("Flush"); 
   digitalWrite(VEG2,LOW);         //OPEN THE SOLENOID TO THE RUN
   delay(240000);                   //WAIT TO FLUSH THE BATCH TO THE RUN..CURRENTLY 4 MINUTES 30 SECONDS  
   delay(2000);                    
   digitalWrite(VEG2,HIGH);        //CLOSE THE SOLENOID AFTER THE BATCH IS DISTRUBUTED  
   digitalWrite (FLUSH_PUMP,HIGH);
*/
//VEG2^
//FLOWER7\/
...
//now we flush the system.
   ...
   digitalWrite(FLUSH_PUMP,HIGH);
      lcd.clear();
   lcd.setCursor(0,0);
   lcd.print ("DELAY BTWN CYCLE");
   lcd.setCursor(0,1);
   lcd.print ("22.5 HOURS"); 
